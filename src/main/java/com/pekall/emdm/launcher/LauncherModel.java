...
public class LauncherModel extends BroadcastReceiver {
    ...
    public interface Callbacks {
        ...
        public void bindHideApps(ArrayList<String> apps);
    }

    LauncherModel(Context app, IconCache iconCache) {
    ...
    private class LoaderTask implements Runnable {
        ...
        List<ResolveInfo> queryInstalledActivities() {
        ...
        ShortcutInfo createShortcutInfo(PackageManager manager, ComponentName cn, int container, int screen, int cellX, int cellY) {
            ShortcutInfo result;
            ...
            result = getShortcutInfo(manager, intent, mContext);
            if (result != null) {
                try {
                    result.intent = Intent.parseUri(intent.toUri(0), 0);
        ...
        private void loadWorkspace() {
            ...
            LauncherApplication.getInstance().getLauncherProvider().loadDefaultFavoritesIfNecessary();
            ...
            synchronized (sBgLock) {
                ...
                for (ResolveInfo resolveInfo : queryInstalledActivities()) {
                    ...
                    info = createShortcutInfo(manager, cn, -100, 1, x++, y);
                    if (info != null) {
                ...
                if (DEBUG_LOADERS) {
                    ...
                    for (int y1 = 0; y1 < mCellCountY; y1++) {
                        String line = "";
                        for (int s = 0; s < Launcher.SCREEN_COUNT; s++) {
                            if (s > 0) {
                                line += " | ";
                            }
                            for (int x1 = 0; x1 < mCellCountX; x1++) {
                                line += ((occupied[s][x1][y1] != null) ? "#" : ".");
                            }
                        }
                        Log.d(TAG, "[ " + line + " ]");
                    }
        ...
        private void bindWorkspace(int synchronizeBindPage) {
            ...
            mDeferredBindRunnables.clear();
            bindWorkspaceItems(oldCallbacks, otherWorkspaceItems, otherFolders,
        ...
        private void loadAllAppsByBatch() {
            ...
            while (i < N && !mStopped) {
                if (i == 0) {
                    ...
                    //cann't use reference, may be another thread is operate this, so here will copy
                    apps = new LinkedList<ResolveInfo>();
                    if (DEBUG_LOADERS) {
