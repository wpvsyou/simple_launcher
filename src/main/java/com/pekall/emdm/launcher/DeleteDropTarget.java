...
public class DeleteDropTarget extends ButtonDropTarget {
    ...
    public void onDragStart(DragSource source, Object info, int dragAction) {
        ...
        //通过隐藏'删除'图标,防止导航栏中删除电话,联系人,短信,安全浏览器图标
        if (info instanceof ShortcutInfo) {
            String className = ((ShortcutInfo) info).intent.getComponent().getClassName();
            if (className.contains("com.pekall.emdm.launcher.hotseatcheat.PhoneCheatActivity")
                    || className.contains("com.pekall.emdm.launcher.hotseatcheat.ContactsCheatActivity")
                    || className.contains("com.pekall.emdm.launcher.hotseatcheat.MessageCheatActivity")
                    || className.contains("com.pekall.emdm.browser.BrowserActivity")) {
