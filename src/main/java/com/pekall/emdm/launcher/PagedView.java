...
//    private View mScrollIndicator;
    ...
    /**
     * when the white app list deploy, the app list will smaller than the page count, but current page is also big, here will set the small one
     * @return
     */
    protected boolean checkAndResetPageIndex(){
        if(mChildRelativeOffsets != null && mCurrentPage >= mChildOffsets.length){
            final int newIndex = mChildOffsets.length-1;
            setCurrentPage(newIndex);
            Log.e(TAG,"checkAndResetPageIndex: new index:"+newIndex);
            return true;
        }
        return false;
    }
    ...
    protected void invalidatePageData(int currentPage, boolean immediateAndOnly) {
        ...
        if (mContentIsRefreshable) {
            ...
            //if the white list change, may the page will smaller than previous, here need must set to the pre page.
